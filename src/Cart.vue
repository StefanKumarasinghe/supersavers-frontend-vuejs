  ...
  <v-app v-if="authenticated">
    <v-container fluid>
      <div class="mx-3 ">
        ...
        <p class="fw-bold">With Grocery Planner, you can plan your trip to the store so you can save on the best deals on the correct items.</p>
        <p class="p-3 fw-bold bg-danger text-white">Deals will finish in: {{ countdown }}, get your deals before it's gone</p>
        ...
        <button @click="shareApp"  class="col-6  text-start col-lg-2 font-weight-bold text-danger">
          ...
          Share the love
            ...
            <v-card flat>
              <v-tabs style="height:auto"
                v-model="tab"
                background-color="transparent"
                color="green"
                grow
              >
              <v-tab style="height:auto"
  v-for="(item, index) in items"
  :key="index"
  class="font-weight-bold"
>
<v-img style="max-width:40px" class="mx-auto d-block p-3" :src="require(`@/assets/${item.image}`)"></v-img>
</v-tab>
</v-tabs>
                  ...
                  <v-expansion-panels v-model="panel1" :disabled="disabled1" multiple flat>
                    ...
                    <v-expansion-panel>
                      ...
                      <v-expansion-panel-content>
                        <div class="pb-3">
                          <v-span class="fw-bold" v-show="Unbought_w.length == 0">No items from Woolworths are added to the item to buy list...</v-span>    
                        </div>
                              ...
                              <div class="row">
                                <div class="col-12 col-md-4 col-lg-5 col-sm-12"> 
                                  <div class="font-weight-bold">
                                    ...
                                    <p class="p-3 bg-danger text-white fw-bold" v-if="shouldShowWarning(list.time)">This product prices may have changed</p>
  ...
  <!-- Remove Button -->
  <v-btn outlined rounded text @click="removeItemFromCart(list.name)" style="border-radius:0" class="font-weight-bold " height="42" width="120">
    <v-icon color="red">
  ...
  <!-- Buy Button -->
  <v-btn rounded @click="boughtItem(list.name)" class="font-weight-bold white--text ms-4" style="border-radius:0"  color="green" height="40" width="50">
    <v-icon>
      mdi-check-circle
    </v-icon>
                    ...
                    <v-expansion-panel>
                      ...
                      <v-expansion-panel-content>
                        <div class="pb-3">
                          <v-span class="fw-bold" v-show="Unbought_c.length == 0">No items from Coles are added to the item to buy list...</v-span>    
                        </div>
    ...
    <v-icon>
      mdi-check-circle
                    ...
                    <v-expansion-panel>
                      ...
                      <v-expansion-panel-content>
                        <div class="pb-3">
                          <v-span class="fw-bold" v-show="Unbought_i.length == 0">No items from IGA are added to the item to buy list...</v-span>    
                        </div>
  ...
  <v-btn rounded @click="boughtItem(list.name)" class="font-weight-bold white--text ms-4" style="border-radius:0"  color="green" height="40" width="50">
    <v-icon>
      mdi-check-circle
                ...
                <!-- Second tab content -->
                <v-tab-item>
                  <v-btn color="primary" class="mx-5 d-block w-100 fw-bold mt-5" @click="toggleAdvancedFilter"><v-icon>mdi-filter</v-icon> Advance Filter</v-btn>

<v-collapse v-if="isAdvancedFilterOpen">
  <v-row class="mb-4 w-100 mx-auto px-5">
    <v-col cols="12">
      <p>Please select a range or a particular day to limit the items you bought</p>
      <v-date-picker v-model="dateRange" range class="mx-auto d-block" label="Date Range"></v-date-picker>
    </v-col>
  </v-row>
</v-collapse>
    <v-expansion-panels v-model="panel2" :disabled="disabled2" multiple flat>
      ...
      <v-expansion-panel v-for="(group, groupName) in groupedBoughtOrders" :key="groupName">
        ...
        <v-expansion-panel-content>
          <div class="pb-3">
            <v-span v-show="group.length === 0">You haven't bought anything...</v-span>
          </div>
          
          <div v-for="(list, i) in group" :key="i" class="d-flex justify-center">
            <div class="row box my-4 py-5 col-lg-12 col-md-12 col-sm-12 col-12">
              <div class="col-12 col-md-2 col-lg-2 col-sm-12 py-0">
                <div class="p-1">
